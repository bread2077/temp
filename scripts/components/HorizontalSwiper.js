export default class HorizontalSwiper {
  constructor(swiperSelector) {
    ...
    this._incrementWidth =
      ...
      (this._swiperElements.length / 2 - 1);
    this._blockHalfWidth =
      this._swiperElements[0].offsetWidth / 2 + this._incrementWidth;
  ...
  _handleTouchMove(e) {
    ...
    if (
      difTouches > this._DETECT_TRESHHOLD &&
      this._n >= 0 &&
      this._n < this._swiperElements.length
    ) {
      if (this._xUp > this._touchCoord.x && this._n > 0) {
        this._swipeLeft();
      } else if (
        this._xUp < this._touchCoord.x &&
        this._n < this._swiperElements.length - 1
      ) {
        this._swipeRight();
      }
    }
