...
export default class ChoiceService {
  constructor(choiceSelector) {
    ...
    this._containerChoiceServices =
      this._choiceBlock.querySelector(".choice__services");
    this._containerChoiceDescription = this._choiceBlock.querySelector(
      ".choice__description"
    );
  }
  ...
  _getChoiceItem(selector) {
    return document
      ...
      .content.querySelector(`.${selector}-container`)
      .cloneNode(true);
  }

  _createChoiceItemWithImage(
    name,
    img,
    service,
    templateSelector,
  ...
  _createChoiceItemWithText(name, img, service, templateSelector) {
    const element = this._createChoiceItemWithImage(
      name,
      img,
      service,
      templateSelector
    ...
    element
      .querySelector(".culc__image")
      .classList.add("culc__image_type_service");
  ...
  _renderServices(e) {
    for (let service in socials[e.target.dataset.name].services) {
      this._choiceSubList.append(
        this._createChoiceItemWithText(
  ...
  _cleanCheckService(section) {
    ...
    if (openedSocial) {
      openedSocial.classList.remove("culc__image-container_active");
    }
  }

  _setCheckService(e) {
    e.target
      .closest(".culc__image-container")
      .classList.add("culc__image-container_active");
  }
  ...
  _showServices(e) {
    if (e.target.classList.contains("culc__image")) {
      ...
      this._containerChoiceDescription.classList.add(
        "choice__description_hided"
      );
  ...
  _showDescriptionService(e) {
    ...
    if (card) {
      ...
      const item =
        socials[cardImage.dataset.name].services[cardImage.dataset.sevice];
      this._choiceAmount.textContent = item.amount;
      this._choiceCost.textContent = item.cost;
      this._containerChoiceDescription.classList.remove(
        "choice__description_hided"
      );
    }
  }
  ...
  setEventListeners() {
    ...
    this._choiceSubList.addEventListener("click", (e) =>
      this._showDescriptionService(e)
    );
  }
}
